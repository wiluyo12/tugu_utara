...
export default function Login() {
    ...
    const handleGoogleLogin = async () => {
        ...
        const { error } = await supabase.auth.signInWithOAuth({
            provider: 'google',
            options: {
                redirectTo: `${getURL()}${redirectTo.startsWith('/') ? redirectTo.slice(1) : redirectTo}`,
            },
        });
        if (error) {
            ...
            <div className={styles.loginCard}>
                ...
                <div className={styles.formSection}>
                    <div className={styles.formContainer}>
                        <h1 className={styles.heading}>{isOtpLogin ? 'Masuk dengan Email' : 'Selamat datang'}</h1>
                        <p className={styles.subHeading}>Jadilah bagian dari kami </p>
